...
const fileUpload = document.getElementById('fileUpload'); // New
...
function connectWebSocket() {
  ...
  ws.addEventListener('open', () => {
    ...
    uploadButton.disabled = false; // Enable upload button on connection
  ...
  ws.addEventListener('close', () => {
    ...
    uploadButton.disabled = true; // Disable upload button on disconnection
...
fileUpload.addEventListener('change', async (event) => {
  ...
  displaySystemMessage(`Attempting to upload file: ${file.name}...`);
  ...
  try {
    ...
    if (response.ok) {
      ...
      displaySystemMessage(`Successfully uploaded and embedded: ${result.filename}. You can now query its content.`);
    } else {
      const errorData = await response.json();
      displaySystemMessage(`File upload failed: ${errorData.detail || 'Unknown error'}`);
    }
  } catch (error) {
    console.error('Error during file upload:', error);
    displaySystemMessage('Network error or server unreachable during file upload.');
  } finally {
