...
from fastapi.staticfiles import StaticFiles
from fastapi.responses import FileResponse
from llms.cli import get_conversation_handle_fn # This might not be suitable for a web app, see below.
from llms.core import TEMP_FOLDER, query_chat_processing_fn
from llms import protected_llm, vulnerable_llm
from websocket_primitives import WSMessage, WSToggleMessage, WSChatMessage
import soteria_sdk # Assuming this is used elsewhere for context or LLM initialization
...
@app.post("/upload-document/")
async def upload_document_endpoint(file: UploadFile): # Make this async for better FastAPI integration
    """
    Handles document uploads via HTTP POST. The uploaded JSON file is saved temporarily,
    embedded into the vector database, and then deleted.
    """
    ...
    try:
        ...
        print(f"Processing file '{file.filename}' with protection mode: {is_protected}")
        ...
        logging.error(f"An unexpected error occurred during file upload or embedding for '{file.filename}': {e}", exc_info=True)
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to process file '{file.filename}': {e}")
    finally:
    ...
    try:
        while True:
            ...
            try:
                ...
                await websocket.send_text(f"System: Invalid message format received.")
                continue
            ...
            if isinstance(message_root, WSToggleMessage):
                ...
                # Fallback for unexpected but valid WSMessage types that aren't WSToggleMessage or WSChatMessage or str
                print(f"[WS_DEBUG] Received unhandled valid WSMessage root type: {type(message_root)} from {websocket.client.host}:{websocket.client.port}. Skipping.")
                await websocket.send_text(f"System: Unhandled message type.")
                continue
        ...
        logging.error(f"WebSocket error for {websocket.client.host}:{websocket.client.port}: {e}", exc_info=True)
...
async def run_llm(websocket: WebSocket, user_input: str) -> str:
    ...
    current_protection_mode = protection_modes.get(websocket, True) # Default to protected

    print(f"[LLM_DEBUG] Calling run_llm for {websocket.client.host}:{websocket.client.port}")
    print(f"[LLM_DEBUG]   User input: '{user_input}'")
    print(f"[LLM_DEBUG]   Resolved protection_mode: {current_protection_mode}")
    print(f"[LLM_DEBUG]   Current context: '{current_context[:100]}...'") # Log first 100 chars of context

    # Select the correct LLM based on the protection mode
    llm_processor = protected_llm.query_chat_processing_fn if current_protection_mode else vulnerable_llm.query_chat_processing_fn

    try:
        # Call the LLM processing function
        new_context, llm_response = await llm_processor(current_context, user_input) # Await the async function
        
        # Update the context for the next turn
        contexts[websocket] = new_context
        return llm_response
    except Exception as e:
        logging.error(f"Error during LLM processing for input '{user_input}': {e}", exc_info=True)
