...
# Set up logging instead of using print with exc_info
...
def load_and_split_data(file_path):
    try:
        ...
        if isinstance(json_data, dict):
            ...
            data = [Document(page_content=content, metadata={"source": file_path, "type": "json_object"})]
            logger.info(f"Processed JSON object with {len(content)} characters")
            ...
            logger.info(f"Processed JSON array with {len(data)} items")
            
        else:
            # Handle primitive types
            content = str(json_data)
            data = [Document(page_content=content, metadata={"source": file_path, "type": "json_primitive"})]
            logger.info(f"Processed JSON primitive: {type(json_data).__name__}")
        
        if not data:
            raise ValueError("No data could be loaded from the JSON file")
            
        logger.info(f"Created {len(data)} documents, now splitting into chunks...")
        ...
        logger.error(f"Error loading and splitting data from {file_path}: {e}", exc_info=True)
        return None
...
def embed(file_or_path):
    ...
    if hasattr(file_or_path, 'filename'):
        ...
        return {"success": False, "error": "Invalid input: expected file object or file path string"}
